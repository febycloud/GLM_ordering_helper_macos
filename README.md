...
MacBook Air M2 / macOS 26 推荐直接双击 `GLM Coding Helper.command`。该入口会使用便携缓存目录，并默认采用 CPU-only 的 1 YOLO worker + 2 OCR worker 配置；监听端口仍保持 `8888`。
...
| `setup-backend-macos.command` | macOS 便携 CPU 环境安装入口 |
