...
这是一个面向智谱 GLM Coding Plan 的抢购辅助项目，包含 Tampermonkey 油猴脚本和本地 CPU OCR 后端，用于限时抢购流程辅助、一键启动后端、中文点选验证码自动识别、验证码自动点击、套餐按钮提前可点、限流重试和多窗口监控。本分支面向 MacBook Air M2 / macOS 26，推荐使用 Chrome。
...
本 Mac 分支推荐直接从你的 GitHub 仓库下载源码或 zip，解压后双击 `GLM Coding Helper.command`。
...
```text
推荐双击：GLM Coding Helper.command
备用 headless 启动：one-click-start.command
```
...
首次和日常使用都双击 `GLM Coding Helper.command`。它会自动安装/修复本地 CPU 后端环境，然后打开 GUI 窗口。如果 macOS 提示无法打开，可以在终端里运行：

```bash
chmod +x "GLM Coding Helper.command" one-click-start.command scripts/setup_backend_macos.sh scripts/portable_env.sh
./"GLM Coding Helper.command"
```
...
2. 下载并解压本仓库，双击 `GLM Coding Helper.command` 启动本地后端。
...
MacBook Air M2 / macOS 26 推荐直接双击 `GLM Coding Helper.command`。该入口会安装/修复本地 CPU 环境、使用便携缓存目录、启动 GUI，并默认采用 CPU-only 的 1 YOLO worker + 2 OCR worker 配置；监听端口仍保持 `8888`。
...
| `GLM Coding Helper.command` | MacBook Air M2 / macOS 26 推荐双击启动入口 |
| `one-click-start.command` | macOS 备用 headless 启动入口 |
...
普通用户优先双击 `GLM Coding Helper.command`。如果你需要手动调试，可以用下面的命令。
