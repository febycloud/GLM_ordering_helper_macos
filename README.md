...
2. 解压
3. 安装油猴脚本
4. 双击 `GLM Coding Helper.command` 启动后端
...
13 英寸 MacBook Air M2 / macOS 26.5 推荐直接双击 `GLM Coding Helper.command`。该入口会安装/修复本地 CPU 环境、使用便携缓存目录、启动 GUI，并默认采用 CPU-only 的 1 YOLO worker + 2 OCR worker 配置；监听端口仍保持 `8888`。
...
| `GLM Coding Helper.command` | 13 英寸 MacBook Air M2 / macOS 26.5 推荐双击启动入口 |
